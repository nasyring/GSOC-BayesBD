...
function (fitted.image, plot.type) 
{
	...
	if(is.string(fitted.image$image)){
		if(substr(fitted.image$image,start = nchar(fitted.image$image)-3,stop = nchar(fitted.image$image))=="jpeg"){
		...
		if(plot.type == 1){
			if(min(intensity)<0){ 
  			...
  			polygon(upper.x, upper.y, fillOddEven = TRUE, col = "gray", border = NA)
   			...
   			lines(estimate.x, estimate.y, lty = 2, lwd = 3)
		...
		if(plot.type == 1){
  			...
  			polygon(upper.x, upper.y, fillOddEven = TRUE, col = "gray", border = NA)
   			polygon(lower.x, lower.y, fillOddEven = TRUE, col = "white", border = NA)
   			lines(estimate.x, estimate.y, lty = 2, lwd = 3)
   			if (!is.null(fitted.image$image$gamma.fun)) {
     				gamma.x = fitted.image$image$gamma.fun(fitted.image$output$theta) * cos(fitted.image$output$theta) + fitted.image$image$center[1]
    				gamma.y = fitted.image$image$gamma.fun(fitted.image$output$theta) * sin(fitted.image$output$theta) + fitted.image$image$center[2]
   				lines(gamma.x, gamma.y, lty = 1, lwd = 1)
			}
