...
gx0 = -1/2 * (sum(log(eigencnini)) + tauinirg * interim) + (alphalambda - 1) * log(lambdaini) - betalambda;
...
while( res1 ){
		...
		res1 =  count(0) < 20.0 ;
	      res2 = L(0)<=0.0;
			...
			interim = anini.t()*(anini / eigencnini);
			g = -0.5 * (sum(log(eigencnini)) + tauini * interim) + (alphalambda - 1) * log(L) - betalambda;
			res3 = g(0) <= logy(0);
		}
            if (res3){ 
			L(0) = L(0)+1.0;
       ...
       while( res1 ){
		...
		res2 = R(0)>=500.0;
			...
			interim = anini.t()*(anini / eigencnini);
			g = -0.5 * (sum(log(eigencnini)) + tauini * interim) + (alphalambda - 1) * log(R) - betalambda;
			res3 = (g(0) <= logy(0));
		}
            if (res3){ 
			R(0) = R(0)-1.0;
    ...
    while(res1) {
		...
		res1 =  count(0) < 100.0  ;
       	if (res2){ 
			...
			interim = anini.t()*(anini / eigencnini);
			g = -0.5 * (sum(log(eigencnini)) + tauini * interim) + (alphalambda - 1) * log(x1) - betalambda;
			res3 = (x1(0) > x0(0));
		}
	  	  	res2 = (g(0) >= logy(0));
        if (res3) {
...
mu1 = Rcpp::rnorm(1,mu0(0)/(nin+1) + nin*xbarin(0)/(nin+1), pow(nin+1.0,-0.5));
mu2 = Rcpp::rnorm(1,mu0(0)/(nout+1) + nout*xbarout(0)/(nout+1),pow(nout+1.0,-0.5));
sdcomp = (intensityobs - (tmpp*mu1(0) + (1.0-tmpp)*mu2(0)))/(tmpp*sd1(0) + (1.0-tmpp)*sd2(0));
std_sum = -sdcomp.t()*sdcomp*0.5;



gx0 = -0.5 * (sum(log(eigencnini)) + tauinirg * interim) + (alphalambda - 1) * log(lambdaini) - betalambda;
...
while( res1 ){												// begin slice sampling of a
		...
		res1 =  count(0) < 100.0 ;
	      res2 = LL(0)<=0.0;
			...
			interim = anini.t()*(anini / eigencnini);
			g = -0.5 * (sum(log(eigencnini)) + tauinirg * interim) + (alphalambda - 1) * log(LL) - betalambda;
			res3 = g(0) <= logy(0);
		}
            if (res3){ 
			LL(0) = LL(0)+1.0;
       ...
       while( res1 ){
		...
		res2 = R(0)>=500.0;
			...
			interim = anini.t()*(anini / eigencnini);
			g = -0.5 * (sum(log(eigencnini)) + tauinirg * interim) + (alphalambda - 1) * log(R) - betalambda;
			res3 = (g(0) <= logy(0));
		}
            if (res3){ 
			R(0) = R(0)-1.0;
    ...
    while(res1) {
		...
		res1 =  count(0) < 200.0  ;
       	if (res2){ 
			...
			interim = anini.t()*(anini / eigencnini);
			g = -0.5 * (sum(log(eigencnini)) + tauinirg * interim) + (alphalambda - 1) * log(x1) - betalambda;
