...
cppFunction(depends = "RcppArmadillo",'Rcpp::List unisliceL(SEXP & ix0, SEXP & igx0, SEXP & ii_J, SEXP & itauini, SEXP & ianini, SEXP & ialphalambda, SEXP & ibetalambda, SEXP & ilambdaini, Rcpp::Function besselI){
RNGScope scope;
...
cppFunction(depends = "RcppArmadillo",'Rcpp::List BayesBDbinary(SEXP & obs, SEXP & inimean, SEXP & nrun, SEXP & nburn, SEXP & J, SEXP & slice, SEXP & outputAll, Rcpp::Function eigenfun, Rcpp::Function besselI, Rcpp::Function unisliceL) { 
...
lambdalist = unisliceL(lambdaini, gx0, i_J, tauinirg, anini,  alphalambda,  betalambda, lambdaini, besselI);
...
cppFunction(depends = "RcppArmadillo",'Rcpp::List BayesBDnormal(SEXP & obs, SEXP & inimean, SEXP & nrun, SEXP & nburn, SEXP & J, SEXP & slice, SEXP & outputAll, Rcpp::Function eigenfun, Rcpp::Function besselI) { 
...
ui = fluidPage(
	...
	selectInput(inputId = "shape", label = "Choose either an elliptical, triangular, or user-supplied boundary, or indicate that the ground truth is unknown.", choices = c("ellipse", "triangle", "file", "unknown")),
	...
	selectInput(inputId = "data_type", label = "Choose to simulate binary or Gaussian data or input data file below.", 
		choices = c("binary sim", "normal sim", "user binary data", "user normal data")),
	...
	numericInput(inputId = "centerx", label = "Input the X-coordinate and Y-coordinate of the reference point interior to the boundary function.", value = 0.5, min = NA, max = NA, step = NA,
  	width = NULL),
 
 	numericInput(inputId = "centery", label = "Y-coordinate of the reference point.", value = 0.5, min = NA, max = NA, step = NA,
  	width = NULL),
...
server = function(input, output){
	...
	data = eventReactive(input$go, {
	 ...
	 if(input$data_type=='binary sim'){
	 	...
	 	obs = par2obs(m = 100, pi.in = input$p_in, pi.out = input$p_out, design = 'J',center, gamma.fun)

	 }else if(input$data_type == 'normal sim'){

		obs = parnormobs(m = 100, mu.in = input$mu_in, mu.out = input$mu_out,sd.in = input$sd_in, sd.out = input$sd_out, design = 'J',center, gamma.fun)
	 ...
	 if(any(input$data_type == 'binary sim',input$data_type == 'user binary data')){
	  	...
	  	cppsamp = BayesBDbinary(obs, .4, input$n_run, input$n_burn, 10, FALSE, FALSE, eigenfun, besselI, unisliceL)

	  }else {

	  	cppsamp = BayesBDnormal(obs, .4, input$n_run, input$n_burn, 10, FALSE, FALSE, eigenfun, besselI)
	  ...
	  return(list(obs = obs, x=x, y=y, lx = lx, ly = ly, ux = ux, uy = uy, gamma.fun = gamma.fun, shape = input$shape))
	})
	...
	output$image = renderPlot({
	  ...
	  if(d$shape!="unknown"){
	  my.radial(d$gamma.fun(theta.plot), theta.plot, line.col = 1, lty = 1, lwd = 1, show.grid = FALSE, add = TRUE)
}
})
}
